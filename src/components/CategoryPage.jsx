    ...
    export default function CategoryPage({ storageLiked }) { //tar imot parameter for å kunne sette ønsker og lagre de i localstorage
        ...
        const categoryEnglish = { //Vår slug er på norsk, dette skjønte vi raskt at ikke fungerte med APIet, det måtte gjøres til engelsk. Vi var da på explorer for å finne ut hvordan dette var i apiet, for deretter og gjøre det om til engelsk og apiets skrivemåte
            musikk: "music",
            sport: "sports",
            teater: "arts & theater"
        };
        const apiCategory = categoryEnglish[slug]; //Her gjøres det som ligger i slug om til engelsk, er musikk valgt så ligger music lagret her som vi kan bruke i fetchen som apiet vil forstå
        ...
        const keywordcheck = keyword ? keyword : `${apiCategory} ${categoryCity}`.trim(); //Ettersom vi ønsket at søkeord skulle fungere uavhengig av alt annet, valgte vi å lage en variabel som holdet på logikken vi ønsket i fetchen. Hvis keyword finnes så gjelder keyword, hvis ikke gjelder valgt kategori og valgt by
        //denne brukte vi også trim() for å sikre at det ikke er mellomrom før og etter ordet ettersom mellomrom er et tegn i programmeringen her

        const handleSearch = () => { //Etter anbefaling fra Ann-Charlott så burde vi lage en søk knapp basert på dette med fetch på filter. Har da laget en søk knapp som da fetcher alt. vi slet i flere dager med dato filtrering på suggest, og bestemte oss derfor for å gi oss og la det være
            ...
            //hva om vi fetcher alt når bruker trykker søk istedet for mange avhengigheter i useeffekten. Vi gjorde derfor det ved å bruke events og suggest. Dette gjorde så events fungerer på alt, venues fungerer på alt utenom dato da suggest ikke har startdatetime parameter. 
            //Attractions fungerer på by men ikke land og dato. Dette fordi attractions ikke har disse filter funksjonene. Dette har vært veldig vanskelig og vi har brukt MYE tid på dette. 

            // Fetcher events under her
            ...
            //Fetcher attractions og venues under her
            fetch(`https://app.ticketmaster.com/discovery/v2/suggest?apikey=AFEfcxa4XlCTGJA56Jk356h0NkfziiWD&keyword=${keywordcheck}&countryCode=${code}`) //bruker suggest på det andre for å få det til på den best mulige måten vi får til med de parameterene tilgjengelig
            .then(response => response.json())
            .then(data => {
                setAttractions(data._embedded?.attractions || []);
                setVenues(data._embedded?.venues || []);
            })
            .catch(error => console.error("Feil ved suggest-fetch", error));
        };

        console.log("venues", venues) // vi konsoll logget venues for å få tak i bilder, vi finner ikke bilder på alle. Hverken her eller i explorer. 

        useEffect(() => {
            if (keyword) { // laget en egen useeffekt for keyword. Hvis keyword eskistere (er true) vil denne useeffekten kjøre. I tillegg vil den kjøre hver gang keyword forandrer seg. Dette sikrer at bruker kan søke flere ting og det dukker opp
                handleSearch();
            }
        }, [keyword]);
        ...
        // Vi hadde først ingen avhengigheter for når den useEffekten skulle kjøre, og da endret det seg ikke når vi trykket fra musikk til sport, da skjønte vi at vi også måtte ha med apiCategory her slik at når apiCategory variabelen forandret seg skulle siden lastes på nytt med nytt innhold
        ...
        return (
            ...
            <section className="filterSearch">
                ...
                <form className="filterSearchForm">
                    ...
                    //onChange sier bare at om noe endrer seg så skal det nye lagres i staten. Også her pratet vi med Ann-Charlott da vi som sagt har slitt veldig med datofiltrering. Det fungerer jo fortsatt ikke som ønsket
                    />
                    </label>
